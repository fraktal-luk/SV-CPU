...
module Frontend(ref InstructionMap insMap, input logic clk, input EventInfo branchEventInfo, input EventInfo lateEventInfo);
    ...
    generate
        ...
        task automatic cachedRedirectLate();
            ...
            predState <= AbstractCore.committedPredState;
            predStateF2 <= AbstractCore.committedPredState;
        ...
        task automatic cachedRedirectBranch();
            ...
            begin
                ...
                predState <= updatePred(causingCP.predState, brEncoding);
                predStateF2 <= updatePred(causingCP.predState, brEncoding);  //  As well
            end
            ...
            case (cachedFetcherState)
                ...
                FS_RUN: begin
                    if (lateEventInfo.redirect)
                        ...
                        // @pred update
                        predState <= updatePred(predState, 'z);

                        stageFetch0 <= stageIP;
                        stageFetch0.predState <= predState;
        ...
        task automatic cachedMoveStagesToF2();
            FrontStage newStageF2 = getFrontStageF2(stageFetch1, expectedTargetF2);
                Mword trgX = TMP_trgFromArr(newStageF2);

            stageFetch1 <= setCacheResponse(stageFetch0, cacheOut, instructionCache.translationSig.padr);
            stageFetch2 <= newStageF2; //getFrontStageF2(stageFetch1, expectedTargetF2);

            if (stageFetch1.active) begin
                logic [1:0] predictionF2 = TMP_getPredictionF2(stageFetch1, expectedTargetF2);
                    Mword trgY = getNextTargetF2(stageFetch1, expectedTargetF2);

                        assert (trgX === trgY) else $error("Targets: %p, %p", trgX, trgY);
                ...
                predStateF2 <= updatePred(predStateF2, predictionF2);
                stageFetch2.predState <= predStateF2; // prediction generated by previous block, if correct, corresponds to adr of new F2
            end
        endtask


            function automatic Mbyte TMP_getPredictionF2(input FrontStage stage, input Mword expectedTarget);
                FrontStage res = stage;
                OpSlotAF arrayF2 = clearBeforeStart(stage.arr, expectedTarget);

                logic anyBranch = 0;
                int brSlot = scanBranches(arrayF2);

                foreach (arrayF2[i]) begin
                    if (arrayF2[i].active && arrayF2[i].branch) anyBranch = 1;
                end

                if (!stage.active) return 'z;
            ...
            function automatic FrontStage getFrontStageF2(input FrontStage fs, input Mword expectedTarget);
                ...
                Mword brTargets[FETCH_WIDTH] = '{default: 'x};

                     //   Mword cmpTarget = getNextTargetF2(fs, expectedTarget);
                ...
                begin
                    ...
                    foreach (fs.arr[i]) begin
                        ...
                        if (branches[i]) brTargets[i] = fs.arr[i].adr + Mword'(ins.sources[1]);
                    end
                end
                ...
                res.arr = arrayF2;
                    ...
                    begin
                        Mword target_N = TMP_trgFromArr(res);

                        //assert (cmpTarget === target) else $error("!!!!!\n %p\n  %p", cmpTarget, target);
                        
                        assert (target_N === target) else begin
                            $error("????\n %p\n  %p", target_N, target);
                        end
                    end
                    ...
                    foreach (fs.arr[i]) begin
                        if (fs.arr[i].takenBranch) begin
                            ...
                            target = fs.arr[i].adr + Mword'(ins.sources[1]);
                               ...
                               // if (target === 0) $error("Yes 0");

                            break;
                        end
                    end

                        //if (target === 0) $error("ast  0");
